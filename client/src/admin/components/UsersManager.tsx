...
const isValidPhone = (v: string) => {
  ...
  return /^0[0-9]{8,9}$/.test(v.replace(/[\s\-()]/g, ""));
};
...
const UsersManager = () => {
  ...
  const fetchUsers = () => {
    axios.get<AdminUser[]>("/api/auth/users")
  ...
  const handleAdd = async () => {
    ...
    if (!isValidPhone(newPhone)) {
      setAddError("מספר טלפון לא תקין (לדוגמה: 052-1234567)");
      return;
    }
    if (!isValidEmail(newEmail)) {
      setAddError("כתובת מייל לא תקינה");
      return;
    }
    setAdding(true);
    try {
  ...
  const handleReset = async () => {
    ...
    if (resetPassword.length < 6) {
      setResetError("סיסמה חייבת להכיל לפחות 6 תווים");
      return;
    }
    setResetting(true);
    try {
      await axios.post(`/api/auth/users/${resetUser.id}/reset-password`, {
  ...
  const handleEdit = async () => {
    ...
    if (!isValidPhone(editPhone)) {
      setEditError("מספר טלפון לא תקין (לדוגמה: 052-1234567)");
      return;
    }
    if (!isValidEmail(editEmail)) {
      setEditError("כתובת מייל לא תקינה");
      return;
    }
    setEditing(true);
  ...
  const handleOwnPassword = async () => {
    if (ownPwNew.length < 6) {
      setOwnPwError("סיסמה חייבת להכיל לפחות 6 תווים");
      return;
    }
    setOwnPwSaving(true);
    try {
  ...
  return (
    <Box>
            ...
            {users.map((u) => (
              <TableRow key={u.id} hover>
                ...
                <TableCell align="center">
                  <Typography
                    variant="body2"
                    color={u.phone ? "text.primary" : "text.disabled"}
                  >
                    {u.phone || "—"}
                  </Typography>
                </TableCell>
      ...
      >
        ...
        >
          <TextField
            ...
            placeholder="052-1234567"
            error={!!editPhone.trim() && !isValidPhone(editPhone)}
            helperText={
              !!editPhone.trim() && !isValidPhone(editPhone)
                ? "מספר טלפון לא תקין"
                : ""
            }
          />
          <TextField
            ...
            placeholder="example@email.com"
            error={!!editEmail.trim() && !isValidEmail(editEmail)}
            helperText={
        ...
        <DialogActions sx={{ px: 3, pb: 2, gap: 1 }}>
          <Button onClick={() => setEditUser(null)} disabled={editing}>
            ביטול
          </Button>
          <Button
            variant="contained"
      ...
      >
        <DialogTitle fontWeight={700}>
          איפוס סיסמה — {resetUser?.username}
        </DialogTitle>
        <DialogContent
          dividers
          sx={{
      ...
      >
        ...
        >
          <TextField
            ...
            type="password"
            value={ownPwNew}
            onChange={(e) => setOwnPwNew(e.target.value)}
            helperText="לפחות 6 תווים"
          />
          {ownPwError && <Alert severity="error">{ownPwError}</Alert>}
        </DialogContent>
        <DialogActions sx={{ px: 3, pb: 2, gap: 1 }}>
          <Button onClick={() => setOwnPwOpen(false)} disabled={ownPwSaving}>
            ביטול
          </Button>
          <Button
            variant="contained"
            onClick={handleOwnPassword}
            disabled={ownPwSaving}
            sx={{ minWidth: 80 }}
          >
            {ownPwSaving ? (
      ...
      >
        ...
        <DialogContent>
          <Typography>האם למחוק את המנהל לצמיתות?</Typography>
        </DialogContent>
        <DialogActions sx={{ px: 3, pb: 2, gap: 1 }}>
